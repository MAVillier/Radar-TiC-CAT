...
ROOT=Path(__file__).resolve().parents[1];D=ROOT/'site/data';VERSION=1
def flat(s):return re.sub(r'\s+',' ',s).strip()
def candidates(d):
 docs=d.get('attachments',[]);out=[]
 for pattern in (r'invitaci|quadre|pcap|administrativ|\bqc\b',r'mem[oò]ria|memoj|justifica'):
  for a in docs:
...
def main():
 ...
 active=[r for r in rows.values() if r['phase']=='Anunci de licitació' and is_open(r) and r['world'] in ('generalitat','local')]
 ...
 def read(a):
  ...
  try:
   ...
   pdf=PdfReader(io.BytesIO(raw));pages=[(i+1,p.extract_text() or '') for i,p in enumerate(pdf.pages[:100])]
   return u,{'version':VERSION,'url':u,'status':'read' if any(t.strip() for _,t in pages) else 'needs_ocr','sha256':hashlib.sha256(raw).hexdigest(),'evidence':extract(pages),'pages':len(pdf.pages),'readPages':len(pages),'fetched':now.isoformat()}

...
export function sourceTime(c){const a=Object.values(c.sourcesUpdated||{}).filter(Boolean).map(Date.parse).filter(Number.isFinite);return a.length?Math.min(...a):Date.parse(c.generated);}
export function dossierModel(r,d={},pdf={}){const verified=pdf.validation&&pdf.validation.recordHash===r.rawHash?pdf.validation.fields||{}:{},t={...d.terms,...verified};for(const field of ['extension','modification']){const flag=field+'Allowed',text=field+'Text',conflict=pdf.validation?.conflicts?.includes(flag),published=d.recordHash===r.rawHash&&d.terms?.[flag]===true&&d.terms?.[text]?.trim();t[flag]=verified[flag]??(!conflict&&published?true:null);t[text]=verified[text]||(!conflict&&published?d.terms[text]:'');}const criteria=(pdf.reviewedCriteria||t.criteria||[]).map(c=>({...c,children:[...(c.children||[])]}));let technical=criteria.find(c=>criteriaType(c)==='technical');const blocks=pdf.technicalBlocks||[];if(!pdf.reviewedCriteria&&technical&&blocks.length&&Math.abs(blocks.reduce((a,b)=>a+b.weight,0)-technical.weight)<.011)technical.children=blocks;
 let extension=verified.extensionAllowed===false?0:verified.extensionAmount??null,modification=verified.modificationAllowed===false?0:verified.modificationAmount??null;

 return {r,d:{...d,terms:t},pdf,criteria,duration:durationLabel(t.duration||r.duration),extension,modification,withExtensions:extension!=null&&r.budget!=null?r.budget+extension:null,vec:t.vec??r.vec,technicalTotal:criteria.filter(c=>criteriaType(c)==='technical').reduce((s,c)=>s+(Number(c.weight)||0),0),facts:pdf.facts||[]};
}

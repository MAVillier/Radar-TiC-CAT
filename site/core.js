...
export function dossierModel(r,d={},pdf={}){const verified=pdf.validation&&pdf.validation.recordHash===r.rawHash?pdf.validation.fields||{}:{},t={...d.terms,...verified};t.extensionAllowed=verified.extensionAllowed??null;t.modificationAllowed=verified.modificationAllowed??null;t.extensionText=verified.extensionText||'';t.modificationText=verified.modificationText||'';const criteria=(pdf.reviewedCriteria||t.criteria||[]).map(c=>({...c,children:[...(c.children||[])]}));let technical=criteria.find(c=>criteriaType(c)==='technical');const blocks=pdf.technicalBlocks||[];if(!pdf.reviewedCriteria&&technical&&blocks.length&&Math.abs(blocks.reduce((a,b)=>a+b.weight,0)-technical.weight)<.011)technical.children=blocks;
